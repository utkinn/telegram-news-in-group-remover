...
func init() {
	registerCommand(
		newCommand(
			"scrutiny",
			"<ник> - Начать пристальное внимание за пользователем",
			func(ctx helpers.ResponseContext) {
				...
				ctx.SendSilentMarkdownFmt("*%s* теперь под _пристальным присмотром_. "+
					"Вытащить его оттуда можно командой /unscrutiny.", userName)
			},
		),
	)
}
